...
articleView.initNewArticlePage = function() {
  ...
  $('#export-field').hide(); // hide for now
  $('#article-json').on('focus', function() {
    $(this).select();
  });
  // TODO: Add an event handler to update the preview and the export field if any inputs change.
  $('#new-form').on('change', articleView.create); // this calls function just below
};
...
articleView.create = function() {
  ...
  var formArticle = new Article({
    ...
    catgegory: $('#article-category').val(),
  ...
  $('#export-field').fadeIn();
  $('#article-json').val(JSON.stringify(formArticle) + ',');  // invoking val is a getter, we've changed it to a setter
};
